    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if x.size > 0:
                ...
                labels[:, 5] = ratio[0] * w * (x[:, 1] - x[:, 3] / 2) + pad[0]  # pad width
                labels[:, 6] = ratio[1] * h * (x[:, 2] - x[:, 4] / 2) + pad[1]  # pad height
                labels[:, 7] = ratio[0] * w * (x[:, 1] + x[:, 3] / 2) + pad[0]
                labels[:, 8] = ratio[1] * h * (x[:, 2] + x[:, 4] / 2) + pad[1]
        ...
        if nL:
            ...
            labels[:, 5:9] = xyxy2xywh(labels[:, 1:5])
...
def load_mosaic_face(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        if x.size > 0:  # Normalized xywh to pixel xyxy format
            ...
            #second bbox
            labels[:, 5] = w * (x[:, 1] - x[:, 3] / 2) + padw
            labels[:, 6] = h * (x[:, 2] - x[:, 4] / 2) + padh
            labels[:, 7] = w * (x[:, 1] + x[:, 3] / 2) + padw
            labels[:, 8] = h * (x[:, 2] + x[:, 4] / 2) + padh
        labels4.append(labels)

    # Concat/clip labels
    if len(labels4):
        labels4 = np.concatenate(labels4, 0)
        np.clip(labels4[:, 1:9], 0, 2 * s, out=labels4[:, 1:9])  # use with random_perspective
        # img4, labels4 = replicate(img4, labels4)  # replicate
        # np.clip(labels4[:, 5:9], 0, 2 * s, out=labels4[:, 5:9])
    # Augment
