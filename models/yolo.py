    ...
    def forward(self, x):
        ...
        if self.export_cat:
            for i in range(self.nl):
                ...
                box_xy1 = (y[:, :, :, :, 5:7] * 2. - 0.5 + self.grid[i].to(x[i].device)) * self.stride[i] # xy1
                box_wh1 = (y[:, :, :, :, 7:9] * 2) ** 2 * self.anchor_grid[i] # wh1
                # box_conf1 = torch.cat((box_xy, torch.cat((box_wh, y[:, :, :, :, 9:10]), 4)), 4)
                ...
                y = torch.cat([box_xy, box_wh, y[:, :, :, :, 4:5], box_xy1, box_wh1, y[:, :, :, :, 4:5], y[:, :, :, :, 10:10+self.nc]], -1)
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                y = torch.full_like(x[i], 0)
                class_range = list(range(5)) + list(range(10,10+self.nc))
                ...
                y[..., 5:7] = (y[..., 5:7] * 2. - 0.5 + self.grid[i].to(x[i].device)) * self.stride[i]  # xy
